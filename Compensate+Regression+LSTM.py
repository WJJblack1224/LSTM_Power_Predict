...
# 對光照度感測計進行回推補償
def Compensate(df):
  # 篩選出 Sunlight(Lux) 小於 117758.2 的資料
  filter_df = df[df['Sunlight(Lux)'] < 117758.2]
  X = filter_df[['Sunlight(Lux)']]
  y = filter_df['Power(mW)']
  # 建立線性迴歸模型
  ...
  df.loc[df['Sunlight(Lux)'] >= 117758.2, 'Sunlight(Lux)'] = (
    (df['Power(mW)'] - intercept) / slope
  ).round(2)
  return df

SourceData = Compensate(SourceData)
...
#============================預測數據============================
#載入模型
regressor = load_model('WheatherLSTM_2024-11-13T21_13_58Z.h5')
Regression = joblib.load('WheatherRegression_2024-11-13T21_14_22Z')
